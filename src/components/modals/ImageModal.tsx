...
export default function ImageModal() {
    ...
    return (
        <motion.div
            ...
            className='fixed z-20 w-full md:w-[75%] top-1/2 left-1/2 p-2 md:p-6 mt-8 -translate-x-1/2 -translate-y-1/2 bg-dark-primary flex flex-col gap-4 rounded-lg'>
            <div className='w-full flex items-center justify-between' onClick={() => setModalImage({ ...modalImage, open: false })}>
                ...
                <X size={24} className='cursor-pointer text-dark-text hover:opacity-80' />
            </div>
            ...
            {!isLoaded && (
                <div className='flex items-center justify-center w-full'>
                    <BeatLoader className='text-bg' color='var(--color-accent)' />
                </div>
            )}
